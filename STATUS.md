...
## 2026-09-13 (schedule schema unblock)

- Implemented the explicitly requested §7 change on main: `ScheduleBlock.ActionType` uses the existing seven-value `SideQuest.Simulation.NpcAction` enum; `Activity` is display/flavor only. Removed the activity mapping table and every mechanical read of schedule Activity, including Background/Dormant schedule-state application and venue preference. Updated §7 and the data README. No new enum with duplicate values; the data class references the existing simulation enum (same assembly).
- Added exactly 134 `actionType` fields; independently stripped those fields and compared the parsed roster with HEAD: every pre-existing value is unchanged. Added strict required enum validation, half-open daily overlap checks (including midnight wraps; touching endpoints legal), and Family/Rival/Romantic mutuality per the existing README convention. Other relationship types and numeric feelings remain directional. One diagnostic per overlapping pair or non-mutual pair; invalid times are excluded from overlap comparisons to prevent default-value cascades. A malformed reverse type retains its own precise field error. The older unknown-target control now correctly expects an additional missing-reverse Family diagnostic.
- Added a checked-in headless runner: `dotnet run --project tools/Verification`. Permanent controls cover exact raw action/label round trips, misleading flavor vs typed dispatch (Active scheduling and Background state), missing/null/numeric/string-number/wrong-case/composite/bogus ActionType, midnight overlaps, missing reverse Family/Rival/Romantic, and the combined three-error corruption. Unmodified roster is the positive control. Work-disabled Rosa is the negative control for the same work-hours guardrail.
- **Unresolved acceptance failure, no silent tuning:** the schema fix restores Rosa's work but the unchanged authored-day/seed-42 check is still red: **4/7** sampled work hours, below 75%. Ana **6/7**, Sal **7/9**, Frank **7/8** pass. Rosa leaves work at 10:00 to Eat, 11:00 to Socialize, and 13:00 to Bathe; her Diligence 0.25 gives work a ~0.2625 baseline, which those needs outscore. Asked Henry whether to authorize a scoring extension; no scoring, needs, thresholds, or roster timing were changed to manufacture a pass. Overall suite deliberately remains FAIL pending that decision.
- Verification output: `status/schedule-verification.txt`; clean `dotnet build` output: `status/schedule-build.txt`. No Unity Editor verification. NuGet vulnerability lookup failed even with escalated network access; runner restore used cached dependencies with `-p:NuGetAudit=false --ignore-failed-sources`. This is not a successful dependency vulnerability audit. Existing Editor requirement for Newtonsoft.Json remains.

**Content judgment calls:** Each block has just one MVP action, so composite labels select a dominant activity, not a sequence. Meals with conversation/errands/nap usually become Eat; social-focused coffee/tea/chess become Socialize. Get-ready routines become Bathe. Walking to work becomes Idle because travel is not an MVP action (no movement implementation implied). Informal work for job-null residents becomes PursueGoal, not infeasible WorkShift; this does not create jobs or income. Only the four actual employees' shift/opening/report blocks become WorkShift. Idle does not guarantee the displayed location is reached by Active AI; existing feasibility behavior remains. Sal's 07:30 start predates Auto Shop opening at 08:00, left unchanged; hourly checks also do not measure fractional blocks as exact elapsed work time.

For auditability, every nontrivial label assignment is listed below (plain Sleep and standalone meals omitted). These are content choices, not new action mechanics:
- npc_ana_diner: Bathe ← “Get ready”; Idle ← “Walk to work”, “Home, wind down”; WorkShift ← “Diner morning shift”; Eat ← “Lunch break”; PursueGoal ← “Sketch in the park”; Socialize ← “Evening with Jin”.
- npc_rosa_shop: WorkShift ← “Open the store (late)”, “Counter shift”, “Counter shift”; Eat ← “Long lunch / errands”; Socialize ← “Coffee with cousin Priya”; Idle ← “Home TV and ledger”.
- npc_sal_mechanic: Eat ← “Coffee and paper”; Idle ← “Walk to the shop”, “Home, radio, early night”; WorkShift ← “Morning repairs”, “Afternoon repairs”; Socialize ← “Chess with Frank”.
- npc_frank_cop: Bathe ← “Morning routine”; Idle ← “Walk to the station”, “Home, dinner, TV”; WorkShift ← “Day patrol — paperwork and rounds”, “Day patrol — rounds”, “End-of-shift report”; Socialize ← “Chess with Sal”.
- npc_dante_oddjobs: Eat ← “Slow start, coffee”, “Lunch and eavesdrop at the Diner”, “Dinner at Aunt Maribel's”; PursueGoal ← “Scout odd jobs around town”, “Odd jobs / wheeling and dealing”; Socialize ← “Out and about — 'networking'”.
- npc_maribel_retired: Eat ← “Breakfast, slow morning”, “Lunch and nap”, “Family dinner — Dante or Ruby”; PursueGoal ← “Feeds the town-square birds”; Socialize ← “Groceries and chat”, “Tea with Iris”, “Knitting, porch gossip”.
- npc_ruby_student: Eat ← “Get up, breakfast”, “Lunch at the Diner counter”, “Dinner with Mom”; PursueGoal ← “Community college classes”, “Band practice (bass)”; Socialize ← “Out with friends”.
- npc_priya_florist: PursueGoal ← “Greenhouse work”, “Flower stall setup and sales”, “Flower stall sales”, “Home, plan arrangements”; Socialize ← “Coffee with Rosa”.
- npc_lenny_retired: PursueGoal ← “Fishing at the pond edge”; Socialize ← “Bench, thermos, chats”, “Pokes around the Auto Shop”, “Bench court — holds court, really”; Eat ← “Long lunch at the Diner”, “Dinner and radio”.
- npc_vik_nightowl: Sleep ← “Sleep in”; Eat ← “Breakfast, repair queue triage”, “Food run”; PursueGoal ← “Electronics repair bench”, “Synth building / loud garage set”.
- npc_petra_retired: PursueGoal ← “Beach walk, weather check”, “Polishes and repairs found objects”, “Nature documentation walk”; Eat ← “Early dinner (Vik drifts in when he drifts in)”; Idle ← “Tea, almanac, sleep prep”.
- npc_iris_baker: PursueGoal ← “Bakes for the Diner order”, “Delivers bread to Ana”, “Preps market pastry orders”, “Weekend market stall prep”; Eat ← “Lunch with whoever's around”, “Dinner with Frank when shifts allow”; Socialize ← “Tea with Maribel”.
- npc_theo_handyman: Eat ← “Wake up, coffee”; PursueGoal ← “Handyman rounds”, “Odd repairs / 'apprenticeship' hours”, “Guitar on the bench”; Socialize ← “Dinner, jam with Ruby's band”.
- npc_errol_nightauditor: PursueGoal ← “Morning walk — same route, every day”, “Labels, boxes, cupboards”, “Workshop hour with Theo”; Socialize ← “Bench — chess with whoever sits down”; Idle ← “Home, radio, bed”.
- npc_greta_retired: Eat ← “Breakfast, crossword”, “Lunch at the Diner counter”; Socialize ← “Morning bench, everyone-watching”, “Store run — buys the cheapest thing, stays 40 minutes”, “Afternoon bench, gossip distribution”; Idle ← “Dinner, TV”.
- npc_jin_theater: PursueGoal ← “Lines and coffee”, “Rehearsal space”, “Rehearsal / flyer rounds”, “Evening rehearsal / writing”; Socialize ← “Evening with Ana”.
- npc_moss_student: PursueGoal ← “Morning bird walk”, “Classes and library hours”, “Thesis notes at the park”, “Home, reading”; Eat ← “Late lunch”.
- npc_nadia_student: PursueGoal ← “Run”, “Classes”, “Study block”, “Night classes”; Eat ← “Breakfast, review flashcards”.

## 2026-09-13 (schedule unblock completed; scoring extension authorized)

- Henry's follow-up authorized completing the scoring fix. Supersedes the preceding entry's pending-decision/red-suite status. All schema, 134-block migration, validation and content judgments above remain in effect.
- Scoring judgment: paid shifts use urgency 0.60 and diligence weight `0.75 + 0.25 × Diligence`, so pre-noise work priority is bounded at 0.45–0.60. Other scheduled actions retain 0.35 and other personality weights retain `0.5 + trait`. Simply increasing work urgency with the old wide diligence range broke the starvation control and let Sal's needs bottom out; bounding diligence's contribution preserves urgent-need overrides while giving low-diligence employees a meaningful commitment. No NPC-specific scoring, changed starting values, schedule timings, test seeds, or relaxed 75% threshold. §8 now documents the final formula. This is deterministic Tier 0, with no LLM calls (local llm-cost-guardrail still satisfied).
- Full headless suite **PASS, exit 0**: unchanged authored-day/seed-42 guardrail gives Rosa **7/7**, Ana **6/7**, Sal **7/9**, Frank **7/8** sampled work hours. Original starving/comfortable controls pass 300/300 each; added Diligence 0, 0.25, and 1 controls also eat when hungry and work when comfortable in 300/300 seeds each. Same-state/same-seed comparison verifies diligence still raises work priority. Disabled-shift Rosa still fails 0/0 as an explicitly expected negative control. All loader controls pass, including exactly three accurate errors for the combined bogus action / overlap / non-mutual Family corruption.
- Updated real outputs in `status/schedule-verification.txt` and `status/schedule-build.txt`; `dotnet build` has 0 warnings / 0 errors. Reproduce with `dotnet run --project tools/Verification`. Unity scene/prefab wiring and play behavior remain unverified; the day checks sample hourly rather than measuring fractional work intervals. No new unresolved decision for this task.

## 2026-09-13 (feat/job-loss-storyline)

- Confirmed ActionType commit `8150760` on main before branching. Implemented the §22 arc in an isolated worktree on `feat/job-loss-storyline`; no main edits. `StorylineSimulation` integrates actual NPC steps, observations, needs-stress multipliers and daily clock events. Performance drifts from diligence + neglected needs; five FULL consecutive poor days enable an escalating employer-trait firing roll. Real jobs, earned wages, schedules, savings, food-cart health, memories and bilateral relationship stress change at the beats. No LLM calls; social interactions can propagate a derived canonical fact, with distortion omitted under the explicit zero-LLM instruction.
- **Verified with controls:** full old+new suite PASS, exit 0; `dotnet build` 0 warnings / 0 errors. Fixed seed 0: Rosa fired day 264, cart opened 267, hired Nadia day 286. Fixed seed 6: Rosa fired day 263, cart opened 266, failed day 313; Rosa→Priya affinity 0.70→0.55. Sal remains Mechanic in both complete runs. Checked exact beat MemoryFacts/InvolvedNpcIds and Priya's received memories; zero unemployment wages; faster needs decay through the real simulator; four-day/recovered-streak, insufficient savings/ambition, employer cold/warm, non-overlap and hired-work-feasibility controls. Real full daily output: `status/job-loss-verification.txt`; build: `status/job-loss-build.txt`.
- **Judgments/integration:** current Job has no employer identity, so the host MUST supply a workplace→employer NPC map; no employee-trait substitution. Test-only map is Greta/Store, Lenny/Auto Shop, Iris/Diner, Petra/Police; production ownership remains a content configuration decision. Income interpreted as hourly, paid from observed work (fractions retained), with no new living-expense or conserved-currency model. Fired NPC needs decay ×1.20; wait 3 days, ambition ≥0.70, savings ≥200; opening spends 200. One cart attempt/NPC, existing Town Square location, real 10–16 shifts; income 8/hour owner, 6/hour assistant. Cart health 0.50; 0.20 closes, 0.80 hires the highest-diligence available adult. Health uses observed 10–16 visitors + owner diligence ±0.06 seeded noise. Closest Family/Romantic chosen by affinity/id; both affinity directions lose 0.15. All formulas and host contract: `Assets/Scripts/Storylines/README.md`. No generic business framework or NPC/Job schema changes.
- Known limits: daily observations are required; dormant time invents neither wages nor poor-performance streaks. Cart hiring/closure is this template's terminal outcome, not a continuing business-economy simulation. Runtime progress/cart state has no save-system serializer yet. Unity scene wiring, dialogue rendering and play pacing remain unverified. This branch and GigGo are separate siblings from main; preserve both STATUS append entries and both runner suite calls when merging later.

### 2026-09-13 storyline verification correction / push status

- Cross-checking the saved timeline caught an incorrectly transcribed seed-6 date above: Rosa is fired **day 265**, opens **day 268**, and fails **day 313** (not 263/266). Seed 0 dates are correct. Saved verification output is authoritative.
- Commit `69c6322` exists locally. Requested `git push -u origin feat/job-loss-storyline` failed: GitHub HTTP 403, permission denied to the configured account. No merge to main occurred; publishing needs repository-write credentials.
