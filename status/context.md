# Context Dump — Side Quest

Raw capture of a full design conversation that happened outside this repo (separate chat with Claude, not a Claude Code session). Pasted in verbatim for reference. **2026-09-11 version** — supersedes the 2026-09-10 capture: §7 (Data Model) is now finalized with real C# types (was pseudocode before), and §20–23 (MVP Town Layout, MVP Job List, First Emergent Storyline, Example GigGo Quests) are new.

**This is not `docs/design-doc.md`.** It hasn't been reconciled or merged into the authoritative design doc yet — see `STATUS.md` for that open item, including a flagged discrepancy about whether that reconciliation has actually happened. The section numbers below (§1–23) are this conversation's own numbering and do not line up with `docs/design-doc.md`'s numbering (which currently ends at §14 with different content at §13/§14 — confirmed unchanged, single commit, as of 2026-09-11).
...
2. **Needs** — hunger, energy, social, money, hygiene, etc. Simple decay-over-time values, exactly like The Sims' motive system.
...
**Not yet scaffolded in this repo as of 2026-09-11** — `Assets/Scripts/` contains no `.cs` source files yet (verified: only a build-tool-generated `AssemblyInfo.cs` under `obj/`, which isn't real source and is gitignored). Turning the two classes above into real files is the next coding step (see `STATUS.md`).
